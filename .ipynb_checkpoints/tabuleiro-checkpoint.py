...
def stop_propriedade(player_name,name):
    # valores da propriedadde
    owner = propriedades[name]['owner']
    price = propriedades[name]['price']
    pay   = propriedades[name]['pay']
    
    if  owner == None:
        ## 1. checar se já tem dono
        eval(player_name).update_money(-price)
        propriedades[name]['owner'] = player_name
        return
